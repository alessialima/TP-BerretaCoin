...
public class Berretacoin {
    ...
    public int maximoTenedor(){
        return heapUsuarios.verMaximo().getId();
    ...
    public void hackearTx(){
        ...
        ListaEnlazada.Nodo<Transaccion> nodo = hackeada.getNodoLista(); 
        ...
        if (hackeada.id_comprador() != 0) { 
            ...
            comprador.setSaldo(comprador.getSaldo() + hackeada.monto());
            actualizarHeap(comprador); // O(logP)
        }
        
        Usuario vendedor = usuarios[hackeada.id_vendedor() - 1];
        vendedor.setSaldo(vendedor.getSaldo() - hackeada.monto());
        ...
        ultimoBloque.getListaTx().eliminarNodo(nodo); // O(1)
        
        if (hackeada.id_comprador() != 0) {
            ultimoBloque.montoTotalTx -= hackeada.monto();
            ultimoBloque.cantTotalTx--;
        }
